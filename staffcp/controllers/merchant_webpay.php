...
class Merchant_webpayController  extends CmsGenerator {
	
	public function index() {
		$db = Register::get('db');
		$sql = "SELECT * FROM ".DB_PREFIX."settings_merchants WHERE `group`='WEBPAY'";
		$this->view->data = $db->query($sql);
		
		if (isset($_POST['save']) && $_POST['save']){
			Logs::addLog(Acl::getAuthedUserId(),'Настройка экварийнга WEBPAY',URL_NOW);
			...
			$this->redirectUrl('/staffcp/merchant_webpay/');
		}
		...
		$sql = "SELECT * FROM ".DB_PREFIX."settings_merchants_result WHERE `merchant` = 'WEBPAY' ORDER BY id DESC LIMIT 0,200";
	...
	private function updateBill($id,$status){
		...
		$db->post("
			UPDATE ".DB_PREFIX."settings_merchants_result SET 
				`status` = '".$status."',
				`check_dt` = '".mktime()."'
			WHERE `merchant` = 'WEBPAY' AND `id` = '".(int)$id."';
		");
	}
}
