...
class Merchant_yandexpcController  extends CmsGenerator {
	
	public function index() {
		$db = Register::get('db');
		$sql = "SELECT * FROM ".DB_PREFIX."settings_merchants WHERE `group`='YANDEXPC'";
		$this->view->data = $db->query($sql);
		
		if (isset($_POST['save']) && $_POST['save']){
			Logs::addLog(Acl::getAuthedUserId(),'Настройка экварийнга ЯндексДеньги PC',URL_NOW);
			...
			$this->redirectUrl('/staffcp/merchant_yandexpc/');
		}
		...
		$sql = "SELECT * FROM ".DB_PREFIX."settings_merchants_result WHERE `merchant` = 'YANDEXPC' ORDER BY id DESC LIMIT 0,200";
	...
	private function updateBill($id,$status){
		...
		$db->post("
			UPDATE ".DB_PREFIX."settings_merchants_result SET 
				`status` = '".$status."',
				`check_dt` = '".mktime()."'
			WHERE `merchant` = 'YANDEXPC' AND `id` = '".(int)$id."';
		");
	}
}
