...
class OfficesController  extends CmsGenerator {
	...
	public function save() {
		...
		if (empty($id)){
			...
			Logs::addLog(Acl::getAuthedUserId(),'Добавление нового офиса',URL_NOW);
		} else {
			$this->model->update($form,array($indexField => $id));
			$this->setMargins($id);
			$this->setIAccess($id);
			Logs::addLog(Acl::getAuthedUserId(),'Редактирование офиса id:'.$id,URL_NOW);
		}
	...
	private function setMargins($account_id=0){
		...
		if (isset($importers) && count($importers)>0){
			...
			$db->post("DELETE FROM ".DB_PREFIX."offices_margin2office WHERE `office_id`='".(int)$account_id."';");
			
			foreach ($importers as $importer_id=>$margin_id){
				if ($importer_id && $margin_id)
				$db->post("INSERT INTO ".DB_PREFIX."offices_margin2office (`office_id`,`importer_id`,`margin_id`) VALUES ('".(int)$account_id."','".(int)$importer_id."','".(int)$margin_id."');");
			}
	...
	private function setIAccess($account_id=0){
		...
		$db->post("DELETE FROM ".DB_PREFIX."accounts_iaccess_offices WHERE `office_id`='".(int)$account_id."';");
		if (isset($iaccess) && count($iaccess)>0){
			foreach ($iaccess as $importer_id){
				if ($importer_id)
				$db->post("INSERT INTO ".DB_PREFIX."accounts_iaccess_offices (`office_id`,`importer_id`) VALUES ('".(int)$account_id."','".(int)$importer_id."');");
			}
		}
	}
	#1#
	...
	private function getMargins($account_id=0){
		$db = Register::get('db');
		$res = $db->query("SELECT * FROM ".DB_PREFIX."offices_margin2office WHERE `office_id`='".(int)$account_id."';");
		$margins = array();
	...
	private function getIAccess($account_id=0){
		$db = Register::get('db');
		$res = $db->query("SELECT importer_id FROM ".DB_PREFIX."accounts_iaccess_offices WHERE `office_id`='".(int)$account_id."';");
		$importer_id = array();
		if (isset($res) && count($res)>0){
			foreach ($res as $dd){
