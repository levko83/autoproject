...
class MultipleType extends Type  {
	...
	public function getFormValue() {
		...
		$result .= '<table border="0" cellspacing="2" cellpadding="0" style="border: 0;">';
		...
		for($i = 0; $i < count($foreignData); $i +=3 )
		{
			...
			$result .= '<tr>';
			$result .= '<td><label><input type="checkbox" name="form['.$fieldName.'][]" '.$selected.' value="'.$value[$foreign['id']].'" /> ID: '.$value[$foreign['id']].' '.$value[$foreign['name']].'</label></td>';
			$result .= '</tr>';
			...
			if (!empty($foreignData[$i + 1]))
			{
				...
				$selected = (in_array($value[$foreign['id']], $selectedValues))?'checked':'';
				$result .= '<td><label><input type="checkbox" name="form['.$fieldName.'][]" '
								.$selected.' value="'.$value[$foreign['id']].'" /> ID: '.$value[$foreign['id']].' '.$value[$foreign['name']].'</label></td>';
			} else {
				$result .= '<td></td>';
			}
			$result .= '</tr>';
