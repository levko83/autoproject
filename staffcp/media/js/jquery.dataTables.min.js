...
$(oSettings.nPaginate).insertAfter(oSettings.nTable);$(oSettings.nPrevious).click(function(){oSettings.iDisplayStart-=oSettings.iDisplayLength;if(oSettings.iDisplayStart<0){oSettings.iDisplayStart=0}
...
fnInit:function(oSettings,fnCallbackDraw){
	...
	/* not view paging if pages only 1 */
	if ((parseInt((oSettings.aiDisplay.length-1)/oSettings.iDisplayLength,10)+1) > 1){
...
fnUpdate:function(oSettings,fnCallbackDraw){var iPageCount=jQuery.fn.dataTableExt.oPagination.iFullNumbersShowPages;
...
}nFilter.className="dataTables_filter";nFilter.innerHTML=oSettings.oLanguage.sSearch.replace("_START_",oSettings.iDisplayStart+1).replace("_END_",oSettings.iDisplayEnd).replace("_TOTAL_",oSettings.aiDisplay.length)+' <input type="text" value="'+oSettings.sPreviousSearch.replace('"',"&quot;")+'" />';
