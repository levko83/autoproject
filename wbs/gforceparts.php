...
class gforceparts extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$client = new SoapClient("http://en.gforceparts.com/Integration.asmx?wsdl");
		$result = $client->logIn(array('login' => $this->config['login'], 'passwd' => $this->config['pass']));
		$result = $client->searchParts(array('query' => $ARTICLE));
		
		if (count($result->searchPartsResult->Part) == 1){
			$arrayData []= $result->searchPartsResult->Part;
		} else {
			$arrayData = $result->searchPartsResult->Part;
		}
