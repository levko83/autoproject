...
class mxgroupru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		/* ~~~~~ */
		$sessionID = file_get_contents("http://zakaz.mxgroup.ru/mxapi/?m=login&login=".$this->config['login']."&password=".$this->config['pass']."");
		$result = (array)simplexml_load_string($sessionID);

		$sID = $result['session'];
		$search = file_get_contents("http://zakaz.mxgroup.ru/mxapi/?session=".$sID."&m=search&zapros=".$ARTICLE."&out=json");
		$data1 = json_decode($search);
		if (isset($data1->result) && $data1->result == 'Out of stock'){
			$data1 = array();
		}
		$sID = $result['session'];
		$search = file_get_contents("http://zakaz.mxgroup.ru/mxapi/?session=".$sID."&m=analog&zapros=".$ARTICLE."&out=json");
		$data2 = json_decode($search);
		if (isset($data2->result) && $data2->result == 'Out of stock'){
			$data2 = array();
		}
		file_get_contents("http://zakaz.mxgroup.ru/mxapi/?session=".$sID."&m=logout");
		...
		if (isset($data['result']) && count($data['result'])>0){
			$dataInDB = array();
			$i=0; foreach ($data['result'] as $dd){ $i++;
				...
				$ARTICLE = strtoupper($dd['articul']);
				$PRICE = ($dd['discountprice']);
				$DESCR = ($dd['name'].' '.$dd['storename']);
				$BOX = (int)$dd['count'];
				$DELIVERY = strtoupper($dd['deliverytime']);
				$WEIGHT = "";
				$IMG_URL = "";
