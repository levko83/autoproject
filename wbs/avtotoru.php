...
class avtotoru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		try {
			...
			$params = array(
				'user_id' => $this->config['login_uid'],
				'user_login' => $this->config['login'],
				'user_password' => $this->config['pass'],	
				'search_code' => $ARTICLE,
				'search_cross' => 'on'
			);
			...
			if (isset($result['Parts']) && count($result['Parts'])>0) {
				...
				$dataInDB = array();
				$i=0; foreach ($result['Parts'] as $dd){ $i++;
				
					$ID = "wbs-avtotoru-".$i;
					$IMPORT_ID = $this->config['importer_id'];
					$BRAND_ID = 0;
					$BRAND_NAME = strtoupper($dd['Manuf']);
					$ARTICLE = strtoupper($dd['Code']);
					$PRICE = strtoupper($dd['Price']);
					$DESCR = $dd['Name'].' '.$dd['Storage'];
					$BOX = strtoupper(($dd['MaxCount'] < 0)?$dd['BaseCount']:$dd['MaxCount']);
					$DELIVERY = strtoupper(array_pop(explode("-", $dd['Delivery'])));
					$WEIGHT = "";
					$IMG_URL = "";
