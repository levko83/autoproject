...
class nextauto extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$url = "http://next-auto.pro/xmlprice.php?login=".$this->config['login']."&password=".$this->config['pass']."&ident=".$GROUP_ID;
		$ch = curl_init($url);
		...
		if ($result){
			...
			for ($i=0;$i<=count($result->detail);$i++){
				...
				$DESCR = strtoupper($dd->caption.' '.$dd->delivery.' '.$dd->stock);
	...
	function FindCatalog($ARTID) {
		try {
			$url = "http://next-auto.pro/xmlprice.php?login=".$this->config['login']."&password=".$this->config['pass']."&code=".FuncModel::stringfilter($ARTID)."&sm=1";
			$ch = curl_init($url);
			curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
			$d = curl_exec($ch);
			$d = mb_convert_encoding($d, 'UTF-8', 'auto');
			$groups = simplexml_load_string($d);
			
			$originalsSearch = array();
			if (isset($groups) && count($groups)>0){
				$i=0; foreach ($groups as $group){ $i++;
				
					$ident = strtoupper($group->ident);
					$articlefull = strtoupper($group->articlefull);
					$producer = strtoupper($group->producer);
				
					$originalsSearch [$i]-> IMP_ID = $this->config['id'];
					$originalsSearch [$i]-> SUP_ID = $ident;
					$originalsSearch [$i]-> ART_ARTICLE_NR = $articlefull;
					$originalsSearch [$i]-> SUP_BRAND = $producer;
					$originalsSearch [$i]-> TEX_TEXT = '';
					$originalsSearch [$i]-> URL = '/search/wbs/?article='.$articlefull.'&brand='.$ident;
