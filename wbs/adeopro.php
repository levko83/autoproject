...
class adeopro extends SearchController {
	...
	function searchAll($serialize=true,$account=array()){
		...
		<message>
			<param>
				...
				<password>'.$this->config['pass'].'</password>
				<code>'.$detailNum.'</code>
				<brand>'.$catalogName.'</brand>
				<sm>1</sm>
			</param>
		</message>
		'; 
		$ch = curl_init();
		$data = array('xml' => $xml);
		curl_setopt($ch, CURLOPT_URL, "http://adeo.pro/pricedetals2.php");
...
// 		var_dump($groups);
		...
		if (isset($groups) && $groups && count($groups)>0){
			$i=0; foreach ($groups as $group){ $i++;
				...
				$originalsSearch [$i]-> SUP_ID = strtoupper($group->producer[0]);
				$originalsSearch [$i]-> ART_ARTICLE_NR = strtoupper($group->article[0]);
				$originalsSearch [$i]-> SUP_BRAND = strtoupper($group->producer[0]);
				$originalsSearch [$i]-> TEX_TEXT = strtoupper($group->ident[0]);
				$originalsSearch [$i]-> URL = '/search/wbs/?article='.strtoupper($group->article[0]).'&brand='.urlencode(strtoupper($group->producer[0]));
