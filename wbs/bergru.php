...
class bergru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$content = file_get_contents("https://api.berg.ru/ordering/get_stock.json?items[0][resource_article]=".$ARTID."&items[0][brand_name]=".$GROUP_ID."&analogs=1&key=".$this->config['login_uid']);
		$result = json_decode($content);
		if (isset($result) && count($result)>0){
			$dataInDB = array();
			$i=0; 
			foreach ($result->resources as $dd){ 
				$i++;
				foreach ($dd->offers as $offer){
					$ID = "wbs-berg-".$dd->id;
					$IMPORT_ID = $this->config['importer_id'];
					$BRAND_ID = 0;
					$BRAND_NAME = strtoupper($dd->brand->name);
					$ARTICLE = strtoupper($dd->article);
					$PRICE = $offer->price;
					$DESCR = $dd->name.' '.$offer->warehouse->name;
					$BOX = $offer->quantity;
	...
	function FindCatalog($detailNum=''){ 
		...
		$url = "https://api.berg.ru/ordering/get_stock.json?items[0][resource_article]=".$detailNum."&analogs=1&key=".$this->config['login_uid'];
		$content = file_get_contents($url);
		$result = json_decode($content);
		$originalsSearch = array();
		if (isset($result->resources) && count($result->resources)>0){
			$i=0; foreach ($result->resources as $group){ $i++;
				$originalsSearch [$i]-> IMP_ID = $this->config['id'];
				$originalsSearch [$i]-> SUP_ID = $group->brand->name;
				$originalsSearch [$i]-> ART_ARTICLE_NR = $group->article;
				$originalsSearch [$i]-> SUP_BRAND = $group->brand->name;
				$originalsSearch [$i]-> TEX_TEXT = $group->name;
				$originalsSearch [$i]-> URL = '/search/wbs/?article='.urlencode($group->article).'&brand='.urlencode($group->brand->name);
