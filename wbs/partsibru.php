...
class partsibru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()){
		...
		if (isset($auth->rows[0]->sessionId) && $auth->rows[0]->sessionId){
			...
			if ($GROUP_ID)
			$items = $this->callPartsib("searchByArticle",array("PHPSESSID"=>$sessionId, "article"=>$ARTICLE, "nomenclature_id"=>$GROUP_ID));
			else 
			$items = $this->callPartsib("searchByArticle",array("PHPSESSID"=>$sessionId, "article"=>$ARTICLE));
			...
			if (isset($items->r) && $items->r == 'B'){
				...
				if (isset($items->rows) && count($items->rows)>0){
					$i=0; foreach ($items->rows as $group){ $i++;
						...
						$originalsSearch [$i]-> SUP_ID = $group['nomenclature_id'];
						$originalsSearch [$i]-> ART_ARTICLE_NR = $detailNum;
						$originalsSearch [$i]-> SUP_BRAND = $group['brand'];
						$originalsSearch [$i]-> TEX_TEXT = $group['title'];
						$originalsSearch [$i]-> URL = '/search/wbs/?article='.strtoupper($detailNum).'&brand='.urlencode($group['nomenclature_id']);
