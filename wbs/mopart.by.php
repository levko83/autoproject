...
class mopartby extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$url = "http://mopart.by/ws/catalog/grm/".$this->config['login_uid']."/".$ARTICLE."/true";
		$get = file_get_contents($url);
		$results = json_decode($get);
		
		$dataInDB = array();
		if (isset($results) && count($results)>0){
			$i=0; foreach ($results as $dd){ $i++;
			
					$dd = (array)$dd;
				
					$ID = "wbs-mopart-".$i;
					$IMPORT_ID = $this->config['importer_id'];
					$BRAND_ID = strtoupper($dd['make']);
					$BRAND_NAME = strtoupper($dd['make']);
					$ARTICLE = strtoupper($dd['part_number']);
					$PRICE = strtoupper($dd['cost']);
					$DESCR = ($dd['description'].' '.$dd['deliv_region']);
					$BOX = strtoupper($dd['nal']);
					$DELIVERY = (int)array_pop(explode("-",$dd['deliv_term']));
					$WEIGHT = "";
					$IMG_URL = "";
