...
class avdmotorsru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$results = file_get_contents("http://www.avdmotors.ru/ws/?action=show&client=".$this->config['login_uid']."&number=".FuncModel::stringfilter($ARTICLE));
		$xml = simplexml_load_string($results);
		if (count($xml)>0){
			$dataInDB = array();
			$i=0; foreach ($xml as $dd){ $i++;
				$dd = (array)$dd;
			
				$ID = "wbs-advmotorsru-".$dd['PriceID'];
				$IMPORT_ID = $this->config['importer_id'];
				$BRAND_ID = 0;
				$BRAND_NAME = strtoupper($dd['CatalogName']);
				$ARTICLE = strtoupper($dd['DetailNum']);
				$PRICE = ($dd['Price']);
				$DESCR = ($dd['DetailNameRus'].' '.$dd['SupplierRegion']);
				$BOX = (int)$dd['Quantity'];
				$DELIVERY = strtoupper($dd['DeliveryPeriod']);
				$WEIGHT = "";
				$IMG_URL = "";
