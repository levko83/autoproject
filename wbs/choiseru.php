...
class choiseru extends SearchController {
...
//		echo ('<pre>');
//		echo ('Время 1: '.$result['time'].' / ');
//		echo ('Кеш: '.(($result['container'][0]['cache'])?'Да':'Нет').' / ');
//		echo ('Время 2: '.$result['container'][0]['time'].' / ');
//		echo ('Кол-во: '.count($result['container'][0]['data']));
//		//var_dump($result['container'][0]['data']);
//		echo ('</pre>');
		...
		if (isset($groups['container'][0]['data']) && count($groups['container'][0]['data'])>0){
			...
			$i=0; foreach ($groups['container'][0]['data'] as $group){ $i++;
				$originalsSearch = (object)array(
					...
					'SUP_ID' => strtoupper(str_replace('"','',$group['producer'])),
					'ART_ARTICLE_NR' => strtoupper($group['code']),
					'SUP_BRAND' => strtoupper(str_replace('"','',$group['producer'])),
					'TEX_TEXT' => $group['caption'],
					'URL' => '/search/wbs/?article='.urlencode($group['code']).'&brand='.urlencode($group['producer']),
				);
