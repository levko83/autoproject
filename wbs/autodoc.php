...
class autodoc extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$ch = curl_init("http://priceservice.autodoc.ru/AutodocServices/Price/price_art?Article=".FuncModel::stringfilter($ARTICLE)."&ClientLogin=".$this->config['login']."&ClientPassword=".$this->config['pass']."&IsCross=1");
		...
		if (count($xml->man_name->price_item)>0){
			$dataInDB = array();
			$i=0; foreach ($xml->man_name->price_item as $dd){ $i++;
				$ID = "wbs-autodoc-".$i;
				...
				$DESCR = strtoupper($dd->part_name.' ('.$dd->dir_name.')');
				$BOX = '+';
