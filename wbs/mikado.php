...
class mikado extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$ch = curl_init("http://www.mikado-parts.ru/ws/service.asmx/Code_Search");
		curl_setopt($ch,CURLOPT_POST,1);
		curl_setopt($ch,CURLOPT_POSTFIELDS,"Search_Code=".$ARTID."&ClientID=".$this->config['login']."&Password=".$this->config['pass']."");
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
		$d = curl_exec($ch);
		$xml = simplexml_load_string($d);
		$dataInDB = array();
		if (isset($xml->List->Code_List_Row) && count($xml->List->Code_List_Row)>0){
			$i=0; foreach ($xml->List->Code_List_Row as $dd){ $i++;
				
					$ID = "wbs-mikado-".strtoupper($dd->ZakazCode[0]);
					$IMPORT_ID = $this->config['importer_id'];
					$BRAND_ID = strtoupper($dd->ProducerBrand[0]);
					$BRAND_NAME = strtoupper($dd->ProducerBrand[0]);
					$ARTICLE = strtoupper($dd->ProducerCode[0]);
					$PRICE = strtoupper($dd->PriceRUR[0]);
					$DESCR = strtoupper($dd->Name[0]);
					$BOX = strtoupper($dd->OnStock[0]);
					$DELIVERY = strtoupper($dd->Srock[0]);
					$WEIGHT = "";
					$IMG_URL = "";
