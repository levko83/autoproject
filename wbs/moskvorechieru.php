...
class moskorechieru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()){
		...
		$ART = isset($_REQUEST['article'])?$_REQUEST['article']:false;
		$BRAND='';
		
		$url = "http://portal.moskvorechie.ru/portal.api?l=".$this->config['login']."&p=".$this->config['pass']."&act=price_by_nr_firm&nr=".$ART."&f=".$BRAND."&alt&name&cs=utf8";
		$result = file_get_contents($url);
		$articles = json_decode($result);
		$result = $articles->result;
		if (count($result)>0){
			$dataInDB = array();
			$i=0; foreach ($result as $dd){ $i++;
				$ID = "wbs-moskvorechieru-".$dd->ID;
				$IMPORT_ID = $this->config['importer_id'];
				$BRAND_ID = 0;
				$BRAND_NAME = strtoupper($dd->brand);
				$ARTICLE = strtoupper($dd->nr);
				$PRICE = $dd->price;
				$DESCR = $dd->name.' ('.$dd->upd.')';
				$BOX = $dd->stock;
	...
	function FindCatalog($ART){
		$url = "http://portal.moskvorechie.ru/portal.api?l=".$this->config['login']."&p=".$this->config['pass']."&act=brand_by_nr&nr=".$ART."&alt&name&cs=utf8";
		$result = file_get_contents($url);
		$catalogs = json_decode($result);
		$groups = $catalogs->result;
		...
		if (isset($groups) && count($groups)>0){
			$i=0; foreach ($groups as $group){ $i++;
				...
				$originalsSearch [$i]-> ART_ARTICLE_NR = $group->nr;
				$originalsSearch [$i]-> SUP_BRAND = $group->brand;
				$originalsSearch [$i]-> TEX_TEXT = $group->name;
				$originalsSearch [$i]-> URL = '/search/wbs/?article='.strtoupper($group->nr).'&brand='.urlencode($group->brand);
