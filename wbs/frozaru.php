...
class frozaru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()){
		...
		$client = new SoapClient("http://www.froza.ru/webservice/search.php?WSDL", array('trace'=>1, "connection_timeout" => 5));
		$params = array();
		$params["login"] = $this->config['login'];
		$params["password"] = $this->config['pass'];
		$params["make_logo"] = "";
		$params["detail_num"] = $ARTICLE;
		$params["find_subs"] = "1";
		$params["sort"] = "";
		$params["currency"] = "";
		...
		if (isset($result_array) && count($result_array)>0){
			$dataInDB = array();
			$i=0; foreach ($result_array as $dd){ $i++;
					$ID = "wbs-frozaru-".$i;
					$IMPORT_ID = $this->config['importer_id'];
					$BRAND_ID = 0;
					$BRAND_NAME = strtoupper($dd->make_name);
					$ARTICLE = strtoupper($dd->detail_num);
					$PRICE = $dd->price;
					$DESCR = $dd->description_rus.' '.$dd->country;
					$BOX = $dd->quantity;
					$DELIVERY = ($dd->delivery_time_guar.' дня');
					$WEIGHT = $dd->weight;
					$IMG_URL = "";
