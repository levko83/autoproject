...
class welovepartsru extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$url = "http://weloveparts.ru.public.api.abcp.ru/search/articles/?userlogin=".$this->config['login']."&userpsw=".md5($this->config['pass'])."&number=".$ARTID."&brand=".$GROUP_ID;
	...
	function FindCatalog($detailNum=''){ 
		...
		$url = "http://weloveparts.ru.public.api.abcp.ru/search/brands/?userlogin=".$this->config['login']."&userpsw=".md5($this->config['pass'])."&number=".$detailNum;
		$content = file_get_contents($url);
		$result = json_decode($content);
