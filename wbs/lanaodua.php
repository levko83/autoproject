...
class lanaodua extends SearchController {
	...
	function searchAll($serialize=true,$account=array()) {
		...
		$host = "http://lana.od.ua/WebService.asmx?WSDL";
		try {
			
			$client = new SoapClient($host);
			$params = array("_num" => $ARTICLE, "login" => $this->config['login'], "password" => $this->config['pass']);
			
			$data = $resData = array();
			
			$res1 = $client->GetPriceOAE($params);
			if (isset($res1->GetPriceOAEResult->result) && count($res1->GetPriceOAEResult->result)==1){
				$resData = $res1->GetPriceOAEResult;
			} else {
				$resData = $res1->GetPriceOAEResult->result;
			}
			if (isset($resData) && count($resData)>0){
				foreach ($resData as $dd){
					$data []= $dd;
				}
			}
			
			$res2 = $client->GetPriceUA2($params);
			if (isset($res2->GetPriceUA2Result->result) && count($res2->GetPriceUA2Result->result)==1){
				$resData = $res2->GetPriceUA2Result;
			} else {
				$resData = $res2->GetPriceUA2Result->result;
			}
			if (isset($resData) && count($resData)>0){
				foreach ($resData as $dd){
					$data []= $dd;
			...
			$res3 = $client->GetPriceUA3($params);
			if (isset($res3->GetPriceUA3Result->result) && count($res3->GetPriceUA3Result->result)==1){
				$resData = $res3->GetPriceUA3Result;
			} else {
				$resData = $res3->GetPriceUA3Result->result;
			}
			if (isset($resData) && count($resData)>0){
				foreach ($resData as $dd){
					$data []= $dd;
			...
			if (count($data)>0){
				...
				$i=0; foreach ($data as $dd){ $i++;
					...
					$BRAND_ID = 0;
					$BRAND_NAME = strtoupper($dd->make);
					$ARTICLE = strtoupper($dd->code);
					$PRICE = strtoupper($dd->price);
					$DESCR = $dd->name;
					$BOX = strtoupper($dd->avaiable);
